...
def run_script(path, payload=None):
    ...
    buf=io.StringIO(); sys.stdout=buf; data=b'' if payload is None else json.dumps(payload, ensure_ascii=False).encode('utf-8'); sys.stdin=FakeIn(data)
...
class HookTests(unittest.TestCase):
    def test_session_and_clean_mode(self):
        out=run_script(SESSION, {'session_id':'size-check'}); self.assertIn('instruction.ctf.md is highest priority', out); self.assertEqual('', run_script(HOOK, {'session_id':'size-check','prompt':'Write a normal React page'}))
